// The sign-in area as pure functions, mirroring the sibling sites' suite:
// the hour, its expiry, and the words the page uses about both.
...
describe('signInView', () => {
  it('offers sign-in and says what it buys when nobody is signed in', () => {
    ...
    expect(view.note).toContain('read your own vault');
  });

  it('is honest that sign-in is the only way in on this site', () => {
    expect(signInView(null, 0).note).toContain('only way in');
  });
  ...
  it('changes tone under five minutes', () => {
    expect(signInView(session, 6).kind).toBe('signed-in');
    expect(signInView(session, 5).kind).toBe('expiring');
    expect(signInView(session, 5).note).toContain('5 minutes left');
  });
...
describe('isSessionExpiry', () => {
  it('recognises auth.ts refusing a call it knows has lapsed', () => {
    expect(
      isSessionExpiry(new ApiError('Not signed in, or the session has expired.', 0, 401))
    ).toBe(true);
  });
...
describe('failureText', () => {
  ...
  it('explains an unreadable vault in vault terms', () => {
    const { title, body } = failureText(new BungieError('private', 'x'));
    expect(title).toBe('This vault is not readable');
    expect(body).toContain('account holder');
  });

  it('still explains an account with no Destiny on it', () => {
    expect(failureText(new BungieError('not-found', 'x')).title).toBe('No Destiny account here');
  });
...
describe('pickOwnMembership', () => {
  ...
  it('falls back to the cross save override guess when no primary is named', () => {
    const picked = pickOwnMembership({
      destinyMemberships: [
...
describe('getOwnPlayer', () => {
  ...
  it('reads the signed-in account with the token', () => {
    return (async () => {
  ...
  it('says so when the Bungie account has no Destiny account on it', async () => {
    ...
    await expect(getOwnPlayer(call)).rejects.toMatchObject({ kind: 'not-found' });
  });
